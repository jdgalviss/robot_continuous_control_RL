...
class Agent():
    ...
    def __init__(self, num_agents, state_size, action_size, random_seed, is_crawler = False):
        ...
        if is_crawler:
            ...
            self.actor_local = ActorCrawler(state_size, action_size, random_seed).to(device)
            self.actor_target = ActorCrawler(state_size, action_size, random_seed).to(device)
            self.actor_optimizer = optim.Adam(self.actor_local.parameters(), lr=1e-4)

            # Critic Network (w/ Target Network)
            self.critic_local = CriticCrawler(state_size, action_size, random_seed).to(device)
            self.critic_target = CriticCrawler(state_size, action_size, random_seed).to(device)
            self.critic_optimizer = optim.Adam(self.critic_local.parameters(), lr=1e-3, weight_decay=WEIGHT_DECAY)
            # Noise process
