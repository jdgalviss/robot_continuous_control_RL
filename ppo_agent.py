...
import math
ENV_PATH = "Crawler_Linux/"

device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

class Agent():
    def __init__(self, env_name, nsteps=200, epochs=10, nbatchs=32,
        ...
        self.env = Environment(train_mode = train_mode, env_path = ENV_PATH + env_name)
        self.num_agents = self.env.num_agents
        self.policy = CrawlerActorCritic(state_size = self.env.state_size, action_size = self.env.action_size).to(device)
        self.nsteps = nsteps
